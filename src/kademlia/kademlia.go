...
func main() {
  ...
  _, err = kadem.DoPing(host, port_uint)
	if err != nil {
		fmt.Println("DoPing Failed...Exit")
		return
	}
...
func executeLine(k *libkademlia.Kademlia, line string) (response string) {
	...
	case toks[0] == "Vanish":
		...
		numberKeys, err := strconv.ParseInt(toks[3], 10, 8)
		if err != nil{
			response = fmt.Sprintf("ERR: invalid numberKeys!")
		}

		threshold, err := strconv.ParseInt(toks[4], 10, 8)
		if err != nil{
			response = fmt.Sprintf("ERR: invalid threshold!")
		}		

		v := k.Vanish(vdoid, data, byte(numberKeys), byte(threshold),0)

		if v.AccessKey != 0{
			response = fmt.Sprintf("OK: vdo is vanished!")
		}else{
			response = fmt.Sprintf("ERR: vdo vanish failed!")
		}


	case toks[0] == "Unvanish":
		if len(toks) != 3{
			response = "Unvanish [nodeID] [vdoID]"
			return
		...
		v := k.Unvanish(nodeid, vdoid)

		if v != nil{
			response = fmt.Sprintf("OK: vdo is unvanished!")
		}else{
			response = fmt.Sprintf("ERR: vdo unvanish failed!")
		}
